	...
	'start':function(){
		$.ajax({
			...
			success: function (e) {
				...
				config.dmrule = 'https://www.meiju56.com/player/dmku/dm_rule.html';//e.data.dmrule;
				...
				if(YZM.ads.state=='on'){
					...
					if(window.location.href.indexOf(".m3u8")>0  || window.location.href.indexOf(".mp4")>0|| window.location.href.indexOf("quan.qq.com")>0){
						YZM.play(config.url);
					}else{
						$.post("api.php", {"url":config.url,"ac":"jx"},function(data) {YZM.type = data.type;if(data.code=="200"){YZM.play(data.url);}else{YZM.play(config.url);}},'json');
					}
				}
			}
		});
...
//   	setTimeout(function() {$("#link1-success").fadeIn(); }, 100);
    	setTimeout(function() {$("#link2").show(); }, 50);
		setTimeout(function() {$("#link3,#span").fadeIn();},100);
	...
	'player':{
		'play':function(url){
			...
			YZM.dp=new yzmplayer({
				...
				theme:config.color,
				logo:config.logo,html5m3u8: true,

				video:{url:url,pic:config.pic,type:YZM.type=='hls'?'hls':'auto',
  				customType: {
